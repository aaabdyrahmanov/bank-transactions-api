...
module.exports = async function validateFormat(req, res, next) {
  ...
  if (lastDate && !isValidDate) {
    return res.status(400).json({
      status: 'failure',
      message: 'Invalid date format! Filtering balances by date would be ignored. Please, try again.'
    })
  }

  const operations = [
    operationData('transactions', true, !!lastTransactionId, 'lastTransactionId', lastTransactionId),
    operationData('balances', false, isValidDate, 'lastDate', lastDate)
  ]
  ...
  */
  function operationData (type, isFatal, hasFilter, filterByName, filterByValue) {
    return {
        type,
        isFatal,
        hasFilter,
